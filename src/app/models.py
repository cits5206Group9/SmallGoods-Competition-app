...
class Competition(db.Model):
    ...
    referees = db.relationship("Referee", backref="competition", lazy=True, cascade="all, delete-orphan")
...
class Lift(db.Model):
    ...
    name = db.Column(db.String(100), nullable=False)  # "Snatch", "Clean & Jerk", "Squat", etc.
...
class Attempt(db.Model):
    ...
    referee_decisions = db.Column(db.JSON)  # {"referee_1": "good_lift", "referee_2": "no_lift", ...}
...
class Score(db.Model):
    ...
    athlete_id = db.Column(db.Integer, db.ForeignKey("athlete.id"), nullable=False)
    event_id = db.Column(db.Integer, db.ForeignKey("event.id"), nullable=False)
    lift_id = db.Column(db.Integer, db.ForeignKey("lift.id"))  # Null for overall event score
