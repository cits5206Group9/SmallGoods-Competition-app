...
def setup_test_data(app):
    with app.app_context():
        ...
        athlete_flight = AthleteFlight(
            athlete_id=athlete.id, flight_id=flight.id, lot_number=1, order=1
        )
        ...
        for i in range(1, 4):
            attempt = Attempt(
                ...
                flight_id=flight.id,  # Add required flight_id
                attempt_number=i,
...
def test_athlete_dashboard_renders(client, app):
    ...
    """
    with app.app_context():
        # Get the athlete ID from the database
        athlete = Athlete.query.first()
        assert athlete is not None, "Test athlete not found"

    # Simulate logged-in athlete by setting session
    with client.session_transaction() as sess:
        sess["athlete_id"] = athlete.id
    ...
    assert "Athlete Dashboard" in html or "Next Attempt" in html


def test_next_attempt_timer_endpoint(client, app):
    """
    ...
    assert "timer_active" in data or "error" in data or "message" in data
...
def test_update_opening_weight(client):
    ...
    """
    payload = {"event_id": "1", "lift_key": "Squat", "weight": "100"}
...
def test_update_reps(client):
    ...
    """
    payload = {"event_id": "1", "lift_key": "Squat", "reps": "[1,1,1]"}
