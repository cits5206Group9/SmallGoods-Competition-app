...
def seed_test_data():
    ...
    with app.app_context():
        ...
        existing_comp = Competition.query.filter_by(
        ...
        day = CompetitionDay(
            competition=competition, day_number=1, date=date.today(), is_active=True
        )
        db.session.add(day)
        db.session.flush()
        ...
        for event_data in events_data:
            category = SportCategory(
                competition_day=day, name=event_data["name"], is_active=True
            )
