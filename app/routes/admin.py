...
from ..utils.referee_generator import (
    generate_sample_referee_data,
    generate_random_username,
    generate_random_password,
)
from ..utils.scoring import (
    ScoringCalculator,
    calculate_scores_after_referee_decision,
    TimerScoring,
)
...
def check_access():
    ...
    if session.get("is_referee"):
        flash(
            "Access denied. Referees can only access their individual interface.",
            "error",
        )
...
def real_time_dashboard():
    ...
    return render_template(
        "admin/real_time_dashboard.html",
...
def display():
    ...
    if current_competition:
        return redirect(
            url_for(
                "display.display_competition", competition_id=current_competition.id
            )
        )
    else:
    ...
    events_data = [
        {
            ...
            "flights": [
                {"id": flight.id, "name": flight.name, "order": flight.order}
    ...
    try:
        ...
        if competition_id:
            ...
            print(
                "Current flights:",
                [
                    (f.id, f.name, f.event_id)
                    for f in Flight.query.filter_by(competition_id=competition_id).all()
                ],
            )
        ...
        existing_flight_ids = {
            flight.id
            for flight in Flight.query.filter_by(competition_id=competition.id).all()
        }
                ...
                for flight_data in event_data["groups"]:
                    ...
                    if flight_id:
                        ...
                        if flight and flight.competition_id == competition.id:
                            ...
                            print(
                                f"Updated flight {flight.id} - {flight.name} for event {event.id}"
                            )
                    else:
                        # Create new flight
                        flight = Flight(
        ...
        if events_to_delete:
            ...
            Event.query.filter(Event.id.in_(events_to_delete)).delete(
        ...
        print(
            ...
            [
                (f.id, f.name, f.event_id)
                for f in Flight.query.filter_by(competition_id=competition.id).all()
            ],
        )
        ...
        if competition.events:
            ...
            athlete_entries = (
                AthleteEntry.query.join(Event, AthleteEntry.event_id == Event.id)
                ...
                for mv in movements:
                    if (mv.get("name") or "").strip() == entry.lift_type:
                        ...
                        if new_default_reps != entry.default_reps:
                            print(
                                f"Updating entry {entry.id}: {entry.lift_type} reps from {entry.default_reps} to {new_default_reps}"
                            )
                            ...
                            entry.attempt_time_limit = int(
                                timer.get("attempt_seconds", 60)
                            )
            ...
            if updated_count > 0:
                print(
                    f"Updated {updated_count} AthleteEntry records with new competition config"
                )
...
def atheletes_management():
    ...
    return render_template(
        "admin/atheletes_management.html",
...
def flights_management():
    try:
        ...
        events = Event.query.options(db.joinedload(Event.competition)).all()

        # Get all athletes with competition information
        athletes = Athlete.query.options(db.joinedload(Athlete.competition)).all()
        ...
        for comp in competitions:
            ...
            for event in comp.events:
                ...
                for flight in event.flights:
                    if flight.event_id == event.id:
                        ...
                        flight_athletes = []
                        if (
                            hasattr(flight, "athlete_flights")
                            and flight.athlete_flights
                        ):
            ...
            for flight in flights:
                if (
                    hasattr(flight, "competition_id")
                    and flight.competition_id == comp.id
                    and not flight.event_id
                ):
                    # Get athletes for this flight
                    flight_athletes = []
            ...
            athletes_data.append(
                {
                    ...
                    "competition_name": athlete.competition.name
        ...
        for flight in flights:
            ...
            if hasattr(flight, "competition_id") and flight.competition_id:
                ...
                competition = next(
                    (c for c in competitions if c.id == competition_id), None
                )
                if competition:
...
def create_athlete():
    ...
    try:
        ...
        athlete = Athlete(
            ...
            bodyweight=float(data.get("bodyweight"))
        ...
        db.session.rollback()
        return jsonify(
            {"status": "error", "message": "Invalid data format: " + str(e)}
        ), 400
    except Exception as e:
        db.session.rollback()
        return jsonify(
            {"status": "error", "message": "Failed to create athlete: " + str(e)}
        ), 500
...
def get_athlete(athlete_id):
    ...
    try:
        ...
        return jsonify(
            {"status": "error", "message": "Failed to retrieve athlete: " + str(e)}
        ), 500
...
def update_athlete(athlete_id):
    ...
    try:
        ...
        if "bodyweight" in data:
            athlete.bodyweight = (
                float(data["bodyweight"]) if data["bodyweight"] else None
            )
        if "competition_id" in data:
            athlete.competition_id = (
                int(data["competition_id"]) if data["competition_id"] else None
            )
        ...
        db.session.rollback()
        return jsonify(
            {"status": "error", "message": "Failed to update athlete: " + str(e)}
        ), 500
...
def delete_athlete(athlete_id):
    ...
    try:
        ...
        print(
            f"Deleted {len(coach_assignments)} coach assignments for athlete {athlete_id}"
        )
        ...
        print(
            f"Deleted {len(athlete_entries)} athlete entries for athlete {athlete_id}"
        )
        ...
        print(
            f"Deleted {len(athlete_flights)} athlete flights for athlete {athlete_id}"
        )
        ...
        return jsonify(
            {"status": "error", "message": "Failed to delete athlete: " + str(e)}
        ), 500
...
def create_user_for_athlete(athlete_id):
    ...
    try:
        ...
        if athlete.user_id:
            ...
            if existing_user:
                return jsonify(
                    {"status": "error", "message": "Athlete already has a user account"}
                ), 400
        ...
        user = User(
            ...
            password_hash=generate_password_hash(
                ""
            ),  # Empty password - athletes login with email only
            first_name=athlete.first_name,
            last_name=athlete.last_name,
        ...
        db.session.rollback()
        return jsonify(
            {"status": "error", "message": "Failed to create user account: " + str(e)}
        ), 500
...
@admin_bp.route(
    "/api/competitions/<int:competition_id>/referee-config", methods=["GET"]
)
def get_referee_config(competition_id):
    ...
    try:
                ...
                if "groups" in event:
                    for group in event["groups"]:
                        if "referee" in group:
                            ...
                            if "n" in ref_config:
                                referee_config["number_of_referees"] = int(
                                    ref_config["n"]
                                )

                            # Parse decision options (new format with objects)
                            if "options" in ref_config and isinstance(
                                ref_config["options"], list
                            ):
                                decision_options = []
                                ...
                                if decision_options:
                                    referee_config["decision_options"] = (
                                        decision_options
                                    )
                ...
                if "movements" in event:
                    for movement in event["movements"]:
                        if "referee" in movement:
                            ...
                            if (
                                "ref_options" in ref_config
                                and ref_config["ref_options"]
                            ):
                                decision_options = []
                                lines = (
                                    str(ref_config["ref_options"]).strip().split("\n")
                                )
                                for line in lines:
                                    if line.strip():
                                        parts = [
        ...
        print(
            f"Final referee config for competition {competition_id}: {referee_config}"
        )
            ...
            [
                {
                    ...
                    "competition_name": ref.competition.name
                    if ref.competition
                    else "Unassigned",
                    ...
                    "last_login": ref.last_login.isoformat()
                    if ref.last_login
                    else None,
                    "notes": ref.notes,
                }
...
def update_referee(referee_id):
    ...
    try:
        ...
        referee.competition_id = data.get(
            "competition_id", referee.competition_id
        )  # Added competition_id update
...
def individual_referee_page(referee_id):
    ...
    referee = Referee.query.get_or_404(referee_id)
    return render_template(
        "admin/individual_referee.html", referee_id=referee_id, referee=referee
    )
...
def referee_login_page(referee_id):
    ...
    referee = Referee.query.get_or_404(referee_id)
    return render_template(
        "admin/referee_login.html", referee_id=referee_id, referee=referee
    )
...
def referee_login():
    ...
    try:
        ...
        if (
            ...
            and referee.password == password
            and referee.is_active
        ):
            ...
            return jsonify(
                {
                    ...
                    "redirect": url_for(
                        "admin.individual_referee_page", referee_id=referee.id
                    ),
                }
            )
        else:
...
def submit_referee_decision():
    ...
    try:
        ...
        decision = data.get(
            "decision"
        )  # Should be decision value like 'good_lift', 'no_lift'
        ...
        referee = Referee.query.filter_by(
            id=referee_id, competition_id=competition_id
        ).first()
        if not referee:
            return jsonify(
                {"success": False, "message": "Invalid referee or competition"}
            ), 404
        ...
        referee_assignment = RefereeAssignment.query.filter_by(
            user_id=virtual_user_id, referee_position=referee.position
        ).first()
        ...
        if attempt_id:
            ...
            if not attempt:
                return jsonify(
                    {"success": False, "message": f"Invalid attempt ID: {attempt_id}"}
                ), 404
            ...
            try:
                ...
                decision_enum = decision_map.get(
                    decision.lower(), AttemptResult.NO_LIFT
                )

            # Create or update referee decision
            existing_decision = RefereeDecision.query.filter_by(
                attempt_id=attempt_id, referee_assignment_id=referee_assignment.id
            ).first()
            ...
            decision_info = (
                f"Decision at {timestamp}: {decision} for attempt {attempt_id}"
            )
            if referee.notes:
                referee.notes += f"\n{decision_info}"
            else:
        ...
        traceback.print_exc()
        return jsonify(
            {"success": False, "message": f"Error submitting decision: {str(e)}"}
        ), 500
...
def get_current_attempt():
    ...
    return jsonify({"status": "waiting", "message": "No active attempt"})
...
def get_decision_results():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "decisions": [d.to_dict() for d in decisions]}
        ), 200
...
def get_decision_filters(competition_id):
    ...
    try:
        ...
        # Get unique events
        events = (
            db.session.query(RefereeDecisionLog.event_name)
        ...
        # Get unique flights
        flights = (
            db.session.query(RefereeDecisionLog.flight_name)
            .filter_by(competition_id=competition_id)
        ...
        athletes = (
            db.session.query(RefereeDecisionLog.athlete_name)
            .filter_by(competition_id=competition_id)
    ...
    try:
        ...
        if "violations" in data:
            ...
            if isinstance(data["violations"], list):
                decision.violations = (
                    ", ".join(data["violations"]) if data["violations"] else None
                )
            else:
...
def delete_decision_result(decision_id):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "message": "Decision deleted successfully"}
        ), 200
...
def api_submit_score():
    ...
    try:
        # request.get_json() intentionally not stored — no processing required currently
...
def get_all_scores():
    ...
    try:
        ...
        for score in scores:
            ...
            flight = (
                Flight.query.get(athlete_entry.flight_id)
                if athlete_entry.flight_id
                else None
            )
...
def auto_generate_referees(competition_id):
    ...
    try:
        ...
        clear_existing = (
            request.get_json().get("clear_existing", False)
            if request.get_json()
            else False
        )
        if clear_existing:
        ...
        if needed_count <= 0:
            ...
            existing_referees = Referee.query.filter_by(
                competition_id=competition_id
            ).all()
            referee_credentials = []
        ...
        return jsonify(
            {"success": False, "message": f"Error generating referees: {str(e)}"}
        ), 500


@admin_bp.route(
    "/api/referees/regenerate-credentials/<int:referee_id>", methods=["POST"]
)
def regenerate_referee_credentials(referee_id):
    ...
    try:
        ...
        db.session.rollback()
        return jsonify(
            {"success": False, "message": f"Error regenerating credentials: {str(e)}"}
        ), 500
...
def create_flight():
    ...
    try:
        ...
        if not data.get("name"):
            return jsonify(
                {"status": "error", "message": "Flight name is required"}
            ), 400
        ...
        if event_id:
            try:
                ...
                return jsonify(
                    {"status": "error", "message": "Invalid event ID format"}
                ), 400
        ...
        if competition_id:
            try:
                ...
                return jsonify(
                    {"status": "error", "message": "Invalid competition ID format"}
                ), 400
        ...
        return jsonify(
            {
                ...
                "flight": {
                    ...
                    "competition_id": flight.competition_id
                    or (
                        flight.event.competition_id
                        if flight.event and flight.event.competition
                        else None
                    ),
                    "competition_name": (
                        flight.competition.name
                        if flight.competition
                        else (
                            flight.event.competition.name
                            if flight.event and flight.event.competition
                            else None
                        )
                    ),
        ...
        db.session.rollback()
        return jsonify(
            {"status": "error", "message": "Failed to create flight: " + str(e)}
        ), 500
...
def reorder_flights():
    ...
    try:
        ...
        db.session.rollback()
        return jsonify(
            {"status": "error", "message": "Failed to update flight order: " + str(e)}
        ), 500
...
def get_flight(flight_id):
    ...
    try:
        ...
        return jsonify(
            {"status": "error", "message": "Failed to retrieve flight: " + str(e)}
        ), 500
...
def update_flight(flight_id):
    ...
    try:
        ...
        db.session.rollback()
        return jsonify(
            {"status": "error", "message": "Failed to update flight: " + str(e)}
        ), 500
...
def delete_flight(flight_id):
    ...
    try:
        ...
        db.session.rollback()
        return jsonify(
            {"status": "error", "message": "Failed to delete flight: " + str(e)}
        ), 500
...
def get_event_flights(event_id):
    ...
    try:
        ...
        return jsonify(
            {"status": "error", "message": "Failed to retrieve flights: " + str(e)}
        ), 500
...
def get_all_flights():
    ...
    try:
        flights = (
            Flight.query.options(joinedload(Flight.event).joinedload(Event.competition))
            .order_by(Flight.order)
            .all()
        )
...
def get_competition_events(competition_id):
    ...
    try:
        competition = Competition.query.get(competition_id)
        if not competition:
            return jsonify({"status": "error", "message": "Competition not found"}), 404
        ...
        return jsonify(
            {"status": "error", "message": "Failed to retrieve events: " + str(e)}
        ), 500
...
def get_available_athletes_for_flight(flight_id):
    ...
    try:
        ...
        if not competition_id:
            subquery = (
                db.session.query(AthleteFlight.athlete_id)
                .filter(AthleteFlight.flight_id == flight_id)
                .subquery()
            )
            available_athletes_query = db.session.query(Athlete).filter(
                ~Athlete.id.in_(subquery), Athlete.is_active
            )
        else:
...
def get_flight_athletes(flight_id):
    ...
    try:
        ...
        flight_athletes_query = (
            db.session.query(Athlete, AthleteFlight)
            .join(AthleteFlight, Athlete.id == AthleteFlight.athlete_id)
            .filter(AthleteFlight.flight_id == flight_id)
            .options(joinedload(Athlete.competition))
...
def get_athlete_attempts(athlete_id):
    ...
    try:
        ...
        entries_query = AthleteEntry.query.options(
            joinedload(AthleteEntry.attempts), joinedload(AthleteEntry.event)
                ...
                attempts_data.append(
                    {
                        ...
                        "sport_type": entry.event.sport_type.value
                        if entry.event and entry.event.sport_type
                        else None,
                    }
                )
        ...
        attempt_numbers = sorted(
            set(attempt["attempt_number"] for attempt in attempts_data)
        )
...
def add_athlete_to_flight(flight_id, athlete_id):
    ...
    try:
        ...
        if not flight or not athlete:
            return jsonify(
                {"status": "error", "message": "Flight or athlete not found"}
            ), 404

        # Check if athlete is already in this flight
        existing = AthleteFlight.query.filter_by(
            flight_id=flight_id, athlete_id=athlete_id
        ).first()
        if existing:
            return jsonify(
                {"status": "error", "message": "Athlete is already in this flight"}
            ), 400

        # Get the next order number for this flight
        max_order = (
            db.session.query(db.func.max(AthleteFlight.order))
            .filter(AthleteFlight.flight_id == flight_id)
            .scalar()
            or 0
        )
        ...
        athlete_flight = AthleteFlight(
            flight_id=flight_id, athlete_id=athlete_id, order=next_order
        )
        ...
        if flight.event:
            ...
            ensure_athlete_entries_for_event(
                athlete_id, flight.event.id, flight_id, next_order
            )
        ...
        db.session.rollback()
        return jsonify(
            {"status": "error", "message": "Failed to add athlete to flight: " + str(e)}
        ), 500


@admin_bp.route(
    "/flights/<int:flight_id>/athletes/<int:athlete_id>", methods=["DELETE"]
)
def remove_athlete_from_flight(flight_id, athlete_id):
    ...
    try:
        ...
        if not athlete_flight:
            return jsonify(
                {"status": "error", "message": "Athlete not found in this flight"}
            ), 404
...
def get_competition_referees_status(competition_id):
    ...
    try:
        ...
        return jsonify(
            {"success": False, "message": f"Error getting referee status: {str(e)}"}
        ), 500
...
def reorder_flight_athletes(flight_id):
    ...
    try:
        ...
        if not athlete_orders:
            return jsonify(
                {"status": "error", "message": "No athlete order data provided"}
            ), 400
        ...
        return jsonify(
            {"status": "success", "message": "Athletes reordered successfully"}
        ), 200

    except Exception as e:
        db.session.rollback()
        return jsonify(
            {"status": "error", "message": "Failed to reorder athletes: " + str(e)}
        ), 500
...
def api_timer_defaults():
    ...
    if flight_id and not (comp_id and event_id):
        ...
        if not comp_id:
            comp_id = (
                fl.event.competition_id if fl.event and fl.event.competition else None
            ) or fl.competition_id
...
def get_flight_attempts_order(flight_id):
    ...
    try:
        Flight.query.get_or_404(flight_id)  # Validate flight exists

        # Get all attempts for this specific flight, ordered by lifting_order
        attempts = (
            db.session.query(Attempt)
            .filter(Attempt.flight_id == flight_id)
            .order_by(
        ...
        if needs_commit:
            ...
            attempts = (
                db.session.query(Attempt)
                .filter(Attempt.flight_id == flight_id)
                .order_by(Attempt.lifting_order.asc(), Attempt.id.asc())
                .all()
            )
            ...
            attempts_data.append(
                {
                    ...
                    "final_result": attempt.final_result.value
        ...
        return jsonify({"status": "success", "attempts": attempts_data}), 200
...
def reorder_flight_attempts(flight_id):
    ...
    try:
        ...
        return jsonify(
            {"status": "success", "message": "Attempt order updated successfully"}
        ), 200

    except Exception as e:
        db.session.rollback()
        return jsonify(
            {"status": "error", "message": "Failed to update attempt order: " + str(e)}
        ), 500
...
def sort_flight_attempts(flight_id, sort_type):
    ...
    try:
        ...
        attempts = (
            db.session.query(Attempt)
            .join(AthleteFlight, Attempt.athlete_id == AthleteFlight.athlete_id)
            .filter(AthleteFlight.flight_id == flight_id)
            .all()
        )
        ...
        db.session.rollback()
        return jsonify(
            {"status": "error", "message": "Failed to sort attempts: " + str(e)}
        ), 500
...
def generate_test_attempts(flight_id):
    ...
    try:
        ...
        if not athlete_flights:
            return jsonify(
                {"status": "error", "message": "No athletes in this flight"}
            ), 400
        ...
        for af in athlete_flights:
            ...
            for attempt_num in range(1, 4):
                ...
                if not existing:
                    ...
                    athlete_entry = (
                        AthleteEntry.query.join(AthleteFlight)
                        .filter(
                    ...
                    base_weight = (
                        50 + (athlete.id % 20) * 5
                    )  # Vary weights based on athlete ID
                    attempt_weight = (
                        base_weight + (attempt_num - 1) * 5
                    )  # Increase each attempt
...
def update_attempt_weight(attempt_id):
    ...
    try:
        ...
        if attempt.attempt_number == 1 and attempt.athlete_entry_id:
            ...
            if athlete_entry:
                athlete_entry.opening_weights = (
                    int(new_weight) if new_weight is not None else 0
                )
        ...
        db.session.rollback()
        return jsonify(
            {"status": "error", "message": "Failed to update attempt weight: " + str(e)}
        ), 500
...
def create_attempt():
    ...
    try:
        ...
        athlete_flight = AthleteFlight.query.filter_by(
            athlete_id=data["athlete_id"], flight_id=data["flight_id"]
        ).first()
        ...
        if existing_attempt:
            return jsonify(
                {
                    "error": f"Attempt {data['attempt_number']} already exists for this athlete in this flight"
                }
            ), 400
        ...
        if not athlete_entry:
            return jsonify(
                {"error": "No athlete entry found for this athlete in this flight"}
            ), 400
        ...
        return jsonify(
            {"message": "Attempt created successfully", "attempt_id": attempt.id}
        )
...
def update_attempt(attempt_id):
    ...
    try:
        ...
        return jsonify(
            {"message": "Attempt updated successfully", "attempt_id": attempt.id}
        )
...
def get_athlete_entry_score(athlete_entry_id):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "athlete_entry_id": athlete_entry_id, "score": score_data}
        )
    except Exception as e:
        return jsonify(
            {"success": False, "message": f"Error calculating score: {str(e)}"}
        ), 500


@admin_bp.route(
    "/api/scoring/athlete-entry/<int:athlete_entry_id>/save", methods=["POST"]
)
def save_athlete_entry_score(athlete_entry_id):
    ...
    try:
        ...
        return jsonify(
            {"success": False, "message": f"Error saving score: {str(e)}"}
        ), 500
...
def get_event_rankings(event_id):
    ...
    try:
        ...
        return jsonify({"success": True, "event_id": event_id, "rankings": rankings})
    except Exception as e:
        return jsonify(
            {"success": False, "message": f"Error calculating rankings: {str(e)}"}
        ), 500
...
def get_flight_rankings(flight_id):
    ...
    try:
        ...
        return jsonify({"success": True, "flight_id": flight_id, "rankings": rankings})
    except Exception as e:
        return jsonify(
            {
                "success": False,
...
def finalize_event_scores(event_id):
    ...
    try:
        ...
        return jsonify(
            {"success": False, "message": f"Error finalizing scores: {str(e)}"}
        ), 500


@admin_bp.route(
    "/api/scoring/athlete/<int:athlete_id>/event/<int:event_id>/total", methods=["GET"]
)
def get_athlete_total_score(athlete_id, event_id):
    ...
    try:
        ...
        return jsonify({"success": True, "total": total_data})
    except Exception as e:
        return jsonify(
            {"success": False, "message": f"Error calculating total score: {str(e)}"}
        ), 500
...
def record_attempt_time(attempt_id):
    ...
    try:
        ...
        if not start_time_str or not end_time_str:
            return jsonify(
                {"success": False, "message": "start_time and end_time required"}
            ), 400
        ...
        return jsonify(
            {"success": False, "message": f"Error recording time: {str(e)}"}
        ), 500
...
def get_attempt_duration(attempt_id):
    ...
    try:
        ...
        if duration is None:
            return jsonify(
                {"success": False, "message": "Attempt timing data not available"}
            ), 404

        return jsonify(
            {"success": True, "attempt_id": attempt_id, "duration": duration}
        )
    except Exception as e:
        return jsonify(
            {"success": False, "message": f"Error getting attempt duration: {str(e)}"}
        ), 500
...
def update_account():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": "An error occurred while updating account"}
        ), 500
...
def get_violations():
    ...
    try:
        ...
        if show_all:
            violations = TechnicalViolation.query.order_by(
                TechnicalViolation.display_order
            ).all()
        else:
            violations = (
                TechnicalViolation.query.filter_by(is_active=True)
                .order_by(TechnicalViolation.display_order)
                .all()
            )

        return jsonify(
            {"success": True, "violations": [v.to_dict() for v in violations]}
        ), 200
...
def create_violation():
    ...
    try:
        ...
        if not name:
            return jsonify(
                {"success": False, "message": "Violation name is required"}
            ), 400
        ...
        if existing:
            return jsonify(
                {"success": False, "message": f'Violation "{name}" already exists'}
            ), 400

        # Get max display_order and add 1
        max_order = (
            db.session.query(db.func.max(TechnicalViolation.display_order)).scalar()
            or 0
        )
        ...
        socketio.emit(
            "violations_updated",
            {"action": "created", "violation": new_violation.to_dict()},
            namespace="/",
        )
...
def update_violation(violation_id):
    ...
    try:
        ...
        if "name" in data:
            ...
            if not new_name:
                return jsonify(
                    {"success": False, "message": "Violation name cannot be empty"}
                ), 400
            ...
            if existing:
                return jsonify(
                    {
                        "success": False,
                        "message": f'Violation "{new_name}" already exists',
                    }
                ), 400
        ...
        socketio.emit(
            "violations_updated",
            {"action": "updated", "violation": violation.to_dict()},
            namespace="/",
        )
...
def delete_violation(violation_id):
    ...
    try:
        ...
        socketio.emit(
            "violations_updated",
            {"action": "deleted", "violation_id": violation_id},
            namespace="/",
        )
...
def toggle_violation(violation_id):
    ...
    try:
        ...
        socketio.emit(
            "violations_updated",
            {"action": "toggled", "violation": violation.to_dict()},
            namespace="/",
        )
