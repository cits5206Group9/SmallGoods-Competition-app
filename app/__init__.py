...
from . import (
    models as models,
)  # Import models so they are registered with SQLAlchemy (explicit re-export for linters)
...
def setup_logging(log_level: str = None) -> None:
    ...
    level_name = (log_level or os.environ.get("FLASK_LOG_LEVEL", "INFO")).upper()
...
def create_app(config_name: str | None = None) -> Flask:
    ...
    if flask_log_level:
        ...
        print("Production mode - using Flask's default logging")
        ...
        def filter(self, record):
            ...
            if hasattr(record, "getMessage"):
                ...
                if (
                    ...
                    and "EIO=4&transport=polling" in message
                    and "200" in message
                ):
                    return False
            return True
    ...
    with app.app_context():
        ...
        # Skip seeding admin user in testing mode
    ...
    try:
        if config.is_sqlite():
            ...
            if config.needs_db_creation(instance_path):
                logger.info(
                    f"Database not found at {db_path}. Creating new database..."
                )
...
def _seed_admin_user() -> None:
    ...
    try:
        ...
        admin_user = User.query.filter_by(
            email="admin@email.com", role=UserRole.ADMIN
        ).first()
