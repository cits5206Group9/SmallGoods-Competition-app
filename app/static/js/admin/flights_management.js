...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const flightAthletesSection = document.getElementById('flight-athletes-section');
    ...
    function bindEvents() {
        ...
        // Cancel buttons
        document.getElementById('cancel-flight-btn').addEventListener('click', closeModals);
        document.getElementById('cancel-delete-flight-btn').addEventListener('click', closeModals);

        // Form submission
        flightForm.addEventListener('submit', handleFlightFormSubmit);

        // Delete confirmation
        document.getElementById('confirm-delete-flight-btn').addEventListener('click', confirmDeleteFlight);

        // Attempt order controls
        document.getElementById('sort-by-weight').addEventListener('click', () => sortAttempts('weight'));
        document.getElementById('sort-by-name').addEventListener('click', () => sortAttempts('name'));
        document.getElementById('randomize-order').addEventListener('click', () => sortAttempts('random'));
    ...
    async function handleCompetitionChange() {
        ...
        try {
            const response = await fetch(`/admin/competitions/${competitionId}/events`);
    ...
    function createFlightCard(flight) {
        ...
        card.innerHTML = `
            ...
            <div class="flight-info">
                <p class="athlete-count"><strong>${flight.athlete_count || 0}</strong> athletes</p>
        ...
        // Bind events
        card.querySelector('.edit-flight').addEventListener('click', handleEditFlight);
        card.querySelector('.manage-athletes').addEventListener('click', handleManageAthletes);
        card.querySelector('.delete-flight').addEventListener('click', handleDeleteFlight);
    ...
    async function handleManageAthletes(event) {
        ...
        document.getElementById('current-flight-order').textContent = `Order: ${flightOrder}`;
    ...
    async function loadFlightAthletes(flightId) {
        try {
            ...
            const availableResponse = await fetch(`/admin/events/${currentEventId}/available-athletes`);
            if (!availableResponse.ok) throw new Error('Failed to load available athletes');
            const availableAthletes = await availableResponse.json();
    ...
    function createAthleteItem(athlete, type) {
        ...
        item.innerHTML = `
            <div class="athlete-info">
                ...
                <div class="athlete-details">
                    ${athlete.team || 'No Team'} • ${athlete.bodyweight || 'No Weight'}kg • ${athlete.gender || 'Unknown'}
                </div>
            </div>
        ...
        if (type === 'available') {
            ...
            actionBtn.addEventListener('click', () => removeAthleteFromFlight(athlete.id));
        }
    ...
    async function addAthleteToFlight(athleteId) {
        try {
            const response = await fetch(`/admin/flights/${currentFlightId}/athletes`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-Requested-With': 'XMLHttpRequest'
            ...
            if (!response.ok) throw new Error('Failed to add athlete to flight');
            ...
            showNotification('Error adding athlete to flight', 'error');
        }
    }

    async function removeAthleteFromFlight(athleteId) {
        try {
            const response = await fetch(`/admin/flights/${currentFlightId}/athletes/${athleteId}`, {
                method: 'DELETE',
                headers: {
                    'X-Requested-With': 'XMLHttpRequest'
                }
            ...
            if (!response.ok) throw new Error('Failed to remove athlete from flight');
            ...
            showNotification('Error removing athlete from flight', 'error');
        }
    }
        ...
        athletes.forEach((athlete, index) => {
            ...
            item.innerHTML = `
                ...
                <div class="attempt-athlete">
                    <div class="attempt-athlete-name">${athlete.first_name} ${athlete.last_name}</div>
                    <div class="attempt-athlete-weight">${athlete.bodyweight || 'No Weight'}kg • ${athlete.team || 'No Team'}</div>
                </div>
    ...
    async function sortAttempts(sortType) {
        ...
        switch (sortType) {
            case 'weight':
                items.sort((a, b) => {
                    const weightA = parseFloat(a.querySelector('.attempt-athlete-weight').textContent) || 0;
                    const weightB = parseFloat(b.querySelector('.attempt-athlete-weight').textContent) || 0;
    ...
    document.getElementById('available-search').addEventListener('input', function() {
        ...
        const athletes = document.querySelectorAll('#available-athletes-list .athlete-item');
        
        athletes.forEach(athlete => {
            const name = athlete.querySelector('.athlete-name').textContent.toLowerCase();
            const details = athlete.querySelector('.athlete-details').textContent.toLowerCase();
