...
from app.models import (
    Attempt, AttemptResult, RefereeDecision, Score, 
    AthleteEntry, ScoringType, Event, Flight
)
...
class ScoringCalculator:
    ...
    def calculate_athlete_entry_score(
        ...
        for attempt in attempts:
            ...
            attempt_scores.append({
                ...
                'weight': float(attempt.actual_weight or attempt.requested_weight or 0),
                'result': attempt.final_result.value if attempt.final_result else 'pending',
                'score': score,
        ...
        if scoring_type == ScoringType.MAX:
            ...
            best_weight = max((s['weight'] for s in attempt_scores if s['score'] > 0), default=0.0)
    ...
    def finalize_event_scores(event_id: int) -> None:
        ...
        scores = Score.query.join(AthleteEntry).filter(
...
class TimerScoring:
    ...
    @staticmethod
    def record_attempt_time(attempt_id: int, start_time: datetime, end_time: datetime) -> float:
