...
def import_competition_model(data):
    ...
    try:
        ...
        day = CompetitionDay(
            competition=competition, day_number=1, date=datetime.now().date()
        )
        db.session.add(day)
        ...
        for event_idx, event_data in enumerate(data.get("events", []), 1):
            ...
            category = SportCategory(competition_day=day, name=event_data["name"])
            ...
            for mov_idx, movement in enumerate(event_data.get("movements", []), 1):
                ...
                exercise = Exercise(
                    ...
                    name=movement["name"],
                    order=mov_idx,
                    attempt_time_limit=movement.get("timer", {}).get(
                        "attempt_seconds", 60
                    ),
                    break_time_default=movement.get("timer", {}).get(
                        "break_seconds", 120
                    ),
                )
